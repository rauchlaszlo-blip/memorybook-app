    ...
    async (pageId: string): Promise<WorkerResult> => {
      ...
      while (state.pending && !state.conflict) {
        ...
        try {
          ...
          console.error(`Mentési hiba a(z) ${pageId} oldalon:`, err);
    ...
    const switchAndLoadPage = async () => {
      ...
      try {
        ...
        if (err?.name !== 'AbortError') {
          console.error('Hiba az oldal betöltése közben:', err);
        }
      } finally {
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          ...
          <button onClick={() => setIsDrawing((prev) => !prev)}>
