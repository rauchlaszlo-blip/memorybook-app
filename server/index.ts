  ...
  const result = await pool.query(
    `UPDATE books
     SET cover_canvas_json = $1,
         ...
         updated_at = CURRENT_TIMESTAMP
     WHERE id = $3 AND owner_user_id = $4 AND book_type IN ('standard', 'event', 'dedication') AND cover_version = $5
  ...
  if (result.rowCount === 0) {
    const check = await pool.query(
      `SELECT cover_version AS "version" FROM books
       WHERE id = $1 AND owner_user_id = $2 AND book_type IN ('standard', 'event', 'dedication')`,
      [bookId, ownerUserId]
    );
...
app.patch('/api/my/books/:bookId/event-settings', async (req, res) => {
  ...
  if (rawEventClosesAt !== null && rawEventClosesAt !== undefined && rawEventClosesAt !== '') {
    const parsed = new Date(rawEventClosesAt);
    if (!Number.isFinite(parsed.getTime())) {
      res.status(400).json({ error: 'INVALID_EVENT_CLOSE_TIME' });
      return;
    }
...
app.post('/api/invites/:token/contributions', publicRateLimit({ windowMs: 60_000, max: 10, scope: 'contribution' }), async (req, res) => {
